...
if __name__ == '__main__':
    ...
    parser.add_argument("--class_name", "--n",
                        help="class name", type=str, required=True)
    parser.add_argument(
        "--input", "--i", help="json file path (labelme)", type=str, required=True)
    parser.add_argument(
        "--output", "--o", help="output file path (coco format)", type=str, required=True)
    parser.add_argument("--join_num", "--j",
                        help="number of join", type=int, required=True)
    parser.add_argument(
    ...
    l2c_train.save_coco_json(train_keypoints, os.path.join(
        saved_coco_path, "keypoints_train.json"))

    # 生成验证集
    l2c_val = Labelme2coco_keypoints(args)
    val_instance = l2c_val.to_coco(val_path)
    l2c_val.save_coco_json(val_instance, os.path.join(
        saved_coco_path, "keypoints_val.json"))

    # 拷贝 labelme 的原始图片到训练集和验证集里面
    # for file in train_path:
    #     shutil.copy(file.replace("json", "bmp"), os.path.join(
    #         saved_coco_path, "coco", "train"))
    # for file in val_path:
    #     shutil.copy(file.replace("json", "bmp"),
